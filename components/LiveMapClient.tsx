...
type RouteStreamPayload = {
  route?: RouteEtaResponse["route"];
};
...
const approximateMetersBetween = (
  from: google.maps.LatLngLiteral,
  to: google.maps.LatLngLiteral
) => {
  const latMeters = (to.lat - from.lat) * 111_000;
  const lngMeters = (to.lng - from.lng) * 111_000 * Math.cos((from.lat * Math.PI) / 180);
...
const useAnimatedLocation = (target: google.maps.LatLngLiteral) => {
  ...
  useEffect(() => {
    ...
    const distanceMeters = approximateMetersBetween(from, to);
    ...
    const durationMs = Math.min(900, Math.max(280, distanceMeters * 18));
    startedAtRef.current = Date.now();
    let frame = 0;

    const tick = () => {
      const progress = Math.min(1, (Date.now() - startedAtRef.current) / durationMs);
...
const useRouteEta = (journey: CareJourney | null) => {
  ...
  const origin = useMemo(
    () =>
      originKey && typeof originLat === "number" && typeof originLng === "number"
        ? {
            lat: originLat,
            lng: originLng
          }
        : null,
    [originKey, originLat, originLng]
  );
  const destination = useMemo(
    () =>
      destinationKey && typeof destinationLat === "number" && typeof destinationLng === "number"
        ? {
            lat: destinationLat,
            lng: destinationLng
          }
        : null,
    [destinationKey, destinationLat, destinationLng]
  );
  const routeKey = useMemo(
    () => (originKey && destinationKey ? `${originKey}:${destinationKey}:${status || "idle"}` : ""),
    [destinationKey, originKey, status]
  );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const activeTracking = ["accepted", "en_route"].includes(status || "");
    const refreshMs = activeTracking ? 30_000 : 60_000;
    const shouldPollRoute = ["accepted", "en_route", "arrived"].includes(status || "");

    if (!shouldPollRoute) {
      return;
    }

    if (bookingId && typeof EventSource !== "undefined") {
      const source = new EventSource(
        `/api/locations/route-stream?bookingId=${encodeURIComponent(bookingId)}`
      );
      ...
      source.addEventListener("route", (event) => {
        ...
        const payload = JSON.parse((event as MessageEvent).data) as RouteStreamPayload;
        if (!payload.route) {
          return;
        }
        ...
        setRoute({
          ...
          decodedPath: payload.route.decodedPath || [],
          cacheSource: payload.route.cacheSource,
  ...
  }, [bookingId, destination, origin, routeKey, status]);
...
function GoogleLiveMap({ journey }: LiveMapProps) {
  ...
  const routePath = useMemo(
    () => liveRoute?.decodedPath || journey?.routePath || [],
    [journey?.routePath, liveRoute?.decodedPath]
  );
